...
import { verifyResetOtp, resendResetOtp } from '@/api/authApi';

type ResetOtpRouteProp = RouteProp<AuthStackParamList, 'ResetOtp'>;
type ResetOtpNavigationProp = NativeStackNavigationProp<AuthStackParamList, 'ResetOtp'>;

const ResetOtpScreen: React.FC = () => {
  const navigation = useNavigation<ResetOtpNavigationProp>();
  ...
  const { email, resetKey } = route.params;
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (response.success) {
        navigation.navigate('SetNewPassword', { email, resetToken: response.data.resetToken });
        return;
      }
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-950">
      <View className="px-6 pt-4">
        <TouchableOpacity className="flex-row items-center gap-2" onPress={() => navigation.navigate('ForgotPassword', { email })}>
